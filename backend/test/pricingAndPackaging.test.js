...
test("TEST 1: Cart = ₹1,500, Regular packaging -> Delivery = ₹99, Packaging = ₹19, Total = ₹1,618", () => {
  ...
  assert.equal(result.packaging.mrp, 29);
...
test("TEST 4: Cart = ₹1,500, Cold packaging -> Delivery = ₹99, Packaging = ₹79, Total = ₹1,678", () => {
  ...
  assert.equal(result.packaging.mrp, 99);
...
test("TEST 10: Security check - Manipulated packaging values fallback safely to regular packaging", () => {
  ...
  assert.equal(manipulated1.mrp, 29);
...
test("EDGE CASE: Empty cart subtotal = 0 has ₹0 delivery and ₹0 packaging fee", () => {
