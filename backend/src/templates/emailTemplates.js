...
export const renderOrderConfirmation = ({ order }) => {
  ...
  const bodyContent = `
    ...
    <table style="width:100%; margin-top:15px; font-size:14px; line-height:1.8;">
      ...
      <tr><td style="color:#64748b;">Delivery Fee</td><td style="text-align:right;">${order.shipping === 0 ? '<span style="color:#16a34a; font-weight:600;">FREE</span>' : `₹${fmtMoney(order.shipping || order.deliveryFee || 0)}`}</td></tr>
      ${order.packaging ? `<tr><td style="color:#64748b;">Handling & Packaging (${order.packaging.name || "Regular"})</td><td style="text-align:right;">₹${fmtMoney(order.packaging.price || 19)}</td></tr>` : `<tr><td style="color:#64748b;">Handling & Packaging</td><td style="text-align:right;">₹19</td></tr>`}
