...
const ProductAttributeIcon = ({ type }) => {
  if (type === "rx") {
    return <Stethoscope size={15} className="text-[#004782] dark:text-[#a4c9ff] shrink-0" />;
  }
  if (type === "coldChain") {
    return <Snowflake size={15} className="text-sky-500 dark:text-sky-400 shrink-0" />;
  }
  if (type === "otc") {
    return <Pill size={15} className="text-[#086b53] dark:text-[#84d6b9] shrink-0" />;
  }
  return null;
};
  ...
  return (
    <div className="space-y-3.5 text-left animate-[fade-in_0.2s_ease-out] flex-1 flex flex-col justify-start gap-3.5 h-full">
      ...
      <div className="flex justify-between items-start gap-sm">
        ...
        {/* Share Button Icon size scaled */}
        <button
          onClick={handleShare}
          className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-zinc-200 hover:bg-slate-55 dark:hover:bg-zinc-850 rounded-full transition-all cursor-pointer shrink-0"
          title="Share Product"
        >
          <Share2 size={15} />
        </button>
      ...
      <div className="flex flex-wrap gap-sm items-center">
        ...
        {product.requiresRx ? (
          <span className="text-[#111827] dark:text-zinc-100 font-medium text-sm flex items-center gap-3 select-none transition-opacity hover:opacity-80">
      ...
      {(product.manufacturer || product.brand) && (
        <div className="pt-sm border-t border-slate-100 dark:border-zinc-800/80 mt-xs">
          <span className="block text-xs font-bold text-[#111827] dark:text-zinc-100 uppercase tracking-wider mb-1 select-none">
            Manufacturer
          </span>
          <p className="text-[#123C7A] dark:text-[#a4c9ff] font-bold uppercase tracking-[0.1em] text-base">
            {product.manufacturer || product.brand}
          </p>
        </div>
      )}
