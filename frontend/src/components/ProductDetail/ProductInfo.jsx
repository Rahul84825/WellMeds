  ...
  return (
    <div className="space-y-4 text-left flex-1 flex flex-col justify-start h-full text-black font-sans">
      ...
      <div className="flex justify-between items-start gap-4">
        <h1 className="text-3xl sm:text-4xl font-bold text-black leading-tight font-sans">
          {product.name}
        </h1>
        <button
          onClick={handleShare}
      ...
      {/* Attribute Badges */}
      <div className="flex flex-wrap gap-3 items-center font-sans text-xs">
        {product.requiresRx ? (
      ...
      {product.molecules && product.molecules.length > 0 && (
        <div className="pt-3.5 border-t border-dashed border-[#c3d4cc] mt-1">
          <span className="block font-sans text-xs font-bold text-black uppercase tracking-wider mb-2 flex items-center gap-1.5 select-none">
            <span className="material-symbols-outlined text-[15px] leading-none text-[#157a6d]">science</span> Active Molecule Composition
          </span>
          <div className="flex flex-wrap items-center font-sans text-sm font-bold text-black break-words max-w-full gap-y-1">
      ...
      {(product.prepaidOnly || product.isNonRefundable) && (
        <div className="pt-3 border-t border-dashed border-[#c3d4cc] mt-1">
          <p className="font-sans text-xs font-bold text-amber-900 bg-amber-50 px-3 py-1.5 rounded-sm border border-amber-300 inline-block">
            {product.prepaidOnly && product.isNonRefundable
