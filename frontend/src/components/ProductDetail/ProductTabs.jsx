    ...
    <div className="space-y-lg select-none">
      {computedSections.map((sec) => {
        return (
          ...
          >
            <h2 className="font-bold text-base md:text-lg text-slate-800 dark:text-zinc-100 flex items-center gap-xs pb-sm border-b border-slate-100 dark:border-zinc-800 uppercase tracking-wider">
              {sec.title}
            </h2>
                ...
                {product.benefits.map((benefit, bIdx) => (
                  <div key={bIdx} className="p-md bg-emerald-500/[0.02] dark:bg-emerald-500/[0.01] rounded-2xl border border-emerald-500/10 flex gap-sm items-start hover:shadow-xs transition-all">
                    <div className="w-6 h-6 rounded-full bg-emerald-500/10 flex items-center justify-center shrink-0">
                      <CheckCircle className="text-[#086b53]" size={16} />
                    </div>
                    <div>
                      <p className="font-bold text-base text-slate-805 dark:text-zinc-200">{benefit.title}</p>
                      {benefit.description && <p className="text-sm text-slate-400 mt-1.5 leading-relaxed">{benefit.description}</p>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Dosage Checklist */}
            {sec.type === "usage" && (
              <ul className="space-y-sm text-base text-slate-655 dark:text-zinc-300 font-medium">
                {product.usageInstructions.map((inst, idx) => (
                  <li key={idx} className="flex gap-sm items-start leading-relaxed">
                    <div className="w-5 h-5 rounded-full bg-[#004782]/10 flex items-center justify-center shrink-0 mt-0.5">
                      <Check className="text-[#004782]" size={14} />
                    </div>
                    <span>{inst}</span>
                  </li>
                ))}
              ...
              <div className="space-y-sm">
                {product.warnings.map((warn, idx) => (
                  <div key={idx} className="p-md bg-red-500/[0.02] border border-red-500/10 rounded-2xl flex gap-sm items-start">
                    ...
                    <p className="text-base text-slate-655 dark:text-zinc-300 leading-relaxed">{warn}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            {sec.type === "sideeffects" && (
              <ul className="space-y-sm text-base text-slate-655 dark:text-zinc-300 font-medium">
                {product.sideEffects.map((side, idx) => (
                  <li key={idx} className="flex gap-sm items-start leading-relaxed">
                    <div className="w-2 h-2 rounded-full bg-red-550 shrink-0 mt-2.5" />
                    <span>{side}</span>
                  </li>
                ))}
              </ul>
            ...
            {sec.type === "storage" && (
              <ul className="space-y-sm text-base text-slate-655 dark:text-zinc-300 font-medium">
                {product.storageInstructions.map((store, idx) => (
                  <li key={idx} className="flex gap-sm items-start leading-relaxed">
                    <div className="w-2 h-2 rounded-full bg-[#086b53] shrink-0 mt-2.5" />
                    <span>{store}</span>
                  </li>
                ))}
              </ul>
            ...
            {sec.type === "safety" && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-md">
                {product.safetyCards.map((card, idx) => {
                  ...
                  return (
                    <div key={idx} className="p-md bg-slate-50/30 dark:bg-zinc-955/10 rounded-2xl border border-slate-100 dark:border-zinc-855 space-y-sm text-base transition-all hover:shadow-xs text-left">
                      <div className="flex justify-between items-center pb-xs border-b border-slate-100 dark:border-zinc-850/80">
                        <span className="font-bold text-slate-800 dark:text-zinc-100 flex items-center gap-xs">
                          <Info size={16} className="text-[#004782]" />
                          {card.title}
                        </span>
                        <span className={`px-2 py-0.5 rounded-full text-[11px] font-black uppercase border tracking-wider ${badgeStyle}`}>
                          {card.status}
                        </span>
                      </div>
                      {card.description && <p className="text-sm text-slate-450 leading-relaxed text-left">{card.description}</p>}
                    </div>
                  );
                })}
              </div>
                  ...
                  return (
                    <div key={idx} className="border border-slate-100 dark:border-zinc-800 rounded-2xl overflow-hidden transition-all bg-slate-50/30 dark:bg-zinc-955/5">
                      <button
                        ...
                        className="w-full flex justify-between items-center p-md font-bold text-slate-700 dark:text-zinc-200 text-base text-left cursor-pointer hover:bg-slate-50 dark:hover:bg-zinc-900/40 transition-colors"
                      >
                        <span className="flex items-center gap-sm">
                          <HelpCircle size={18} className="text-[#004782] shrink-0" />
                          {faq.question}
                        </span>
                      ...
                      <div className={`transition-all duration-350 ease-in-out overflow-hidden ${
                        ...
                        <p className="text-sm text-slate-450 dark:text-zinc-400 leading-relaxed">
                          {faq.answer}
                        </p>
                      </div>
                    </div>
            ...
            {/* References */}
            {sec.type === "references" && (
              <ul className="space-y-xs text-sm text-slate-455 italic">
            ...
            {!sec.type && (
              <p className="text-base text-slate-655 dark:text-zinc-300 leading-relaxed whitespace-pre-line text-left font-medium">
                {sec.content}
              </p>
            )}
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800/80 p-lg rounded-3xl shadow-sm space-y-md text-left">
        <h2 className="font-bold text-base md:text-lg text-slate-800 dark:text-zinc-100 flex items-center gap-xs pb-sm border-b border-slate-100 dark:border-zinc-800 uppercase tracking-wider">
          <Building size={18} className="text-[#004782]" />
          Manufacturer Information
        </h2>
        <div className="flex flex-col sm:flex-row gap-lg items-start text-base text-left">
          <div className="w-16 h-16 rounded-2xl bg-slate-50 dark:bg-zinc-955 flex items-center justify-center shrink-0 border border-slate-100 dark:border-zinc-850">
            <Building className="text-[#004782]" size={36} />
          </div>
          <div className="space-y-sm flex-1">
            <h3 className="font-bold text-base text-slate-855 dark:text-zinc-100">{product.manufacturer || product.brand}</h3>
            <p className="text-slate-400 font-medium">Manufacturing License Verified</p>
            <p className="text-sm text-slate-505 dark:text-zinc-450 leading-relaxed">
      ...
      <div className="bg-gradient-to-br from-[#004782] to-[#086b53] text-white p-lg rounded-3xl shadow-md text-left flex flex-col md:flex-row items-center justify-between gap-lg relative overflow-hidden">
        ...
        <div className="space-y-xs z-10">
          <h3 className="text-lg font-black flex items-center gap-xs uppercase tracking-wider">
        ...
        <div className="flex gap-sm z-10 shrink-0 select-none">
          <span className="bg-white/10 border border-white/10 px-md py-1 rounded-xl text-xs font-bold uppercase tracking-wider">Licensed Hub</span>
          <span className="bg-white/10 border border-white/10 px-md py-1 rounded-xl text-xs font-bold uppercase tracking-wider">Cold Chain</span>
        </div>
      </div>
    </div>
  );
