    ...
    <aside className="w-full lg:w-[26%] lg:sticky lg:top-24 space-y-md order-3 max-w-[380px] text-xs select-none">
      <div className="bg-white dark:bg-zinc-900 border border-slate-150 dark:border-zinc-855/60 py-md px-sm rounded-3xl shadow-lg space-y-md">
        ...
        <div className="bg-slate-50/50 dark:bg-zinc-955/20 py-2.5 px-3 rounded-2xl border border-slate-100 dark:border-zinc-855 text-left">
          <span className="text-sm font-bold text-slate-400 uppercase tracking-wider">Best Price</span>
          ...
          {product.originalPrice && product.originalPrice > product.price && (
            <div className="flex flex-wrap gap-xs items-center mt-1">
              <span className="bg-emerald-500 text-white font-black text-xs px-2.5 py-1 rounded-full uppercase tracking-wider shadow-xs">
                {discountPercent}% OFF
              </span>
              <span className="text-sm font-bold text-[#086b53] dark:text-emerald-400">
                Save {formatCurrency(product.originalPrice - product.price)}
              </span>
            </div>
          )}
          <p className="text-sm text-slate-400 mt-1 font-medium">Inclusive of all taxes & GST</p>
        </div>

        {/* Quantity Selector */}
        {(product.inStock !== false && product.stock > 0) ? (
          <div className="space-y-1.5 text-left">
            <span className="block text-sm font-bold text-slate-400 uppercase tracking-wider">Select Quantity</span>
            <div className="flex items-center border border-slate-200 dark:border-zinc-800 rounded-2xl bg-slate-50/50 dark:bg-zinc-900 h-11 w-full justify-between p-1">
              ...
              <span className="font-extrabold text-lg text-on-surface">{quantity}</span>
              <button
                type="button"
        ...
        <div className="space-y-2 pt-0.5">
          <button
            ...
            className="w-full bg-[#086b53] hover:bg-[#055746] text-white font-bold h-11 rounded-2xl transition-all hover:shadow-md active:scale-98 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-sm outline-none cursor-pointer text-base shadow-sm"
          >
            Buy Now
          </button>
          <button
            ...
            disabled={product.inStock === false || product.stock === 0}
            className="w-full border-2 border-[#004782] text-[#004782] dark:text-[#a4c9ff] dark:border-[#a4c9ff]/50 hover:bg-[#004782]/5 font-bold h-11 rounded-2xl transition-all active:scale-98 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-sm outline-none cursor-pointer text-base"
          >
            Add to Cart
          </button>
        </div>
        ...
        <div className="text-left bg-slate-50 dark:bg-zinc-955/20 py-2.5 px-3 rounded-2xl border border-slate-100 dark:border-zinc-855 space-y-xs">
          <p className="font-bold text-sm text-slate-550 flex items-center gap-xs">
            <Truck size={16} className="text-[#086b53]" /> Delivery Estimate
          </p>
          <p className="text-sm font-bold text-slate-700 dark:text-zinc-300">Free delivery by Tomorrow, 2:00 PM</p>
        </div>

        {/* Trust badges */}
        <div className="space-y-2 pt-sm border-t border-slate-150 dark:border-zinc-855 text-left">
          <div className="flex items-center gap-sm text-sm font-bold text-slate-400">
