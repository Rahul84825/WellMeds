...
const SubstituteProducts = ({ substituteProducts = [], product }) => {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 p-sm lg:p-md rounded-2xl shadow-xs select-none w-full flex flex-col gap-sm">
      ...
      {!hasSubstitutes ? (
        <div className="text-center py-6 text-xs text-slate-400 dark:text-zinc-500 font-medium">
        ...
        <>
          ...
          <div className="flex flex-col gap-xs w-full">
            {substituteProducts.slice(0, 3).map((item) => {
              ...
              return (
                ...
                >
                  {/* Product Name */}
                  <h4 className="font-semibold text-sm lg:text-base text-slate-850 dark:text-zinc-200 line-clamp-2 leading-tight">
                    {item.name}
                  </h4>
                  
                  {/* Price section */}
                  <div className="flex items-baseline gap-xs mt-1 flex-wrap leading-none">
                    <span className="font-bold text-xs lg:text-sm text-slate-800 dark:text-zinc-200">
                      {formatCurrency(item.price)}
                    </span>
                    {item.originalPrice && item.originalPrice > item.price && (
                      <>
                        <span className="text-[10px] lg:text-xs text-slate-400 line-through">
                          MRP {formatCurrency(item.originalPrice)}
                        </span>
                        <span className="text-[10px] lg:text-xs font-bold text-emerald-600 dark:text-emerald-400">
                          {itemDisc}% OFF
                        </span>
                      </>
                    )}
          ...
          <button
            ...
            className="flex items-center justify-center w-full h-9 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-[#004782] to-[#038076] hover:opacity-90 transition-all active:scale-[0.98] mt-xs cursor-pointer shadow-sm shadow-[#038076]/10"
          >
            View All →
          </button>
        </>
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-zinc-800 pb-md mb-md">
              <h3 className="font-extrabold text-lg text-slate-800 dark:text-zinc-150 uppercase tracking-wider">
                Substitute Products
              </h3>
              <button
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-sm sm:text-base text-slate-850 dark:text-zinc-205 truncate">
                        {item.name}
                      </h4>
                      {item.manufacturer && (
                        <p className="text-xs text-slate-450 dark:text-zinc-500 mt-[2px] font-medium">
                          {item.manufacturer}
                        </p>
                      )}
                    </div>
