...
/**
 * HealthSupplementsSection — WellMeds Design System V2
 * Dedicated homepage section rendering products toggled as "Health Supplements".
 */
const HealthSupplementsSection = () => {
  ...
  useEffect(() => {
    ...
    const fetchSupplements = async () => {
      try {
        const data = await api.getProducts({ isHealthSupplement: true, limit: 12 });
        if (isMounted) {
          setProducts(data.products || []);
          setLoading(false);
        }
      } catch (err) {
        console.error("Failed to fetch health supplements section products:", err);
        if (isMounted) {
          setError(true);
          setLoading(false);
        }
  ...
  if (error || products.length === 0) {
    return null; // Silent hide if no products toggled as Health Supplements
  }
      ...
      <div className="home-section-container">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
          <div className="text-left">
            ...
            <p className="font-sans text-xs sm:text-sm text-[#3f544d] dark:text-zinc-400 mt-1 font-normal">
              Doctor-recommended vitamins, minerals, and daily nutritional supplements.
            </p>
          </div>
        ...
        <div className="relative w-full">
          ...
          <button
            ...
            aria-label="Previous Supplements"
          >
            <ChevronLeft size={20} />
          </button>
...
export default React.memo(HealthSupplementsSection);
