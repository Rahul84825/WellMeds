...
export const LocationProvider = ({ children }) => {
  const [selectedLocation, setSelectedLocation] = useState(() => {
    try {
      ...
      const userSelectedFlag = localStorage.getItem("wellmeds_location_user_selected") === "true";
      if (saved) {
        ...
        if (parsed && (parsed.pincode || parsed.city || parsed.state || parsed.name || parsed.displayText)) {
          ...
          const isUserSelected = Boolean(parsed.isUserSelected || userSelectedFlag || (parsed.pincode && parsed.pincode !== "411021"));
  ...
  const setLocation = useCallback((loc) => {
    ...
    try {
      ...
      localStorage.setItem("wellmeds_location_user_selected", "true");
    } catch (e) {
  ...
  const isLocationAdded = Boolean(
    selectedLocation?.isUserSelected ||
    (selectedLocation?.pincode && selectedLocation?.pincode !== "411021") ||
    (typeof window !== "undefined" && localStorage.getItem("wellmeds_location_user_selected") === "true")
  );
