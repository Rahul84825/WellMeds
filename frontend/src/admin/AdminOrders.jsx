...
const ManageOrders = () => {
  ...
  const handleStatusChange = async (orderId, status) => {
    try {
      ...
      // Update local state using orderId as key
      setOrders((prev) =>
        prev.map((o) => (o.orderId === orderId || o._id === orderId ? { ...o, status } : o))
      );
    } catch (err) {
  ...
  const handleVerifyRx = async (orderId) => {
    try {
      ...
      setOrders((prev) =>
        prev.map((o) => (o.orderId === orderId || o._id === orderId ? { ...o, status: "Approved" } : o))
      );
      setRxModalOpen(false);
    } catch (err) {
      console.error("Failed to approve order prescription", err);
    ...
    <div className="space-y-xl animate-[fade-in_0.3s_ease-out] text-left">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant dark:divide-outline/40 text-body-sm text-on-surface-variant dark:text-surface-variant">
              {orders.map((order) => {
                ...
                const isRxVerified = rxVerifiedOrders[displayId];
                const displayDate = formatDate(order.createdAt);
                return (
                  <tr key={displayId} className="hover:bg-surface-container-low/30 transition-colors">
                    <td className="p-md font-bold text-on-surface font-mono text-xs">{displayId}</td>
                    ...
                    <td className="p-md">
                      <select
                        value={order.status}
                        onChange={(e) => handleStatusChange(displayId, e.target.value)}
                        className={`text-xs font-bold rounded-lg border-none focus:ring-1 py-1 pl-2 pr-6 cursor-pointer dark:bg-inverse-surface ${
                          order.status === "Delivered"
                            ...
                            ? "bg-error-container/20 text-error focus:ring-error"
      ...
      {selectedOrderForRx && (
        <Modal
          ...
          title={`Verify Prescription: Order ${selectedOrderForRx.orderId}`}
          maxWidth="max-w-md"
        >
          <div className="space-y-md text-left">
            <div className="bg-surface-container p-md rounded-xl space-y-sm border border-outline-variant/60">
              ...
              <p className="text-body-sm text-on-surface-variant flex items-center gap-xs">
                ...
                Date Uploaded: <span className="font-medium text-on-surface">{formatDate(selectedOrderForRx.createdAt)}</span>
              </p>
            </div>
            ...
            <div className="flex gap-sm pt-sm border-t border-outline-variant">
              ...
              <button
                type="button"
                onClick={() => handleVerifyRx(selectedOrderForRx.orderId)}
