  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const totalSalesVal = orders.reduce((sum, o) => sum + o.total, 0) + 128450.00; // Adding dummy baseline
        const pendingRxCount = orders.filter(o => o.rxUploaded && o.status === "Processing").length;
        ...
        setStats({
          ...
          activeViews: 3241,
          pendingRxVerification: pendingRxCount
        });
      } catch (err) {
        ...
        <div className="lg:col-span-2 bg-surface-container-lowest dark:bg-inverse-surface border border-outline-variant dark:border-outline/40 rounded-xl p-lg shadow-sm space-y-md">
          ...
          <div className="relative w-full h-64 bg-surface-container-low dark:bg-surface-container rounded-lg p-md overflow-hidden flex items-end">
            <svg className="absolute inset-0 w-full h-full p-lg" viewBox="0 0 100 100" preserveAspectRatio="none">
              ...
              {/* Trend line */}
              <path d="M 0,90 Q 20,60 40,75 T 80,30 T 100,20" fill="none" stroke="#004782" strokeWidth="1.5" strokeLinecap="round" className="dark:stroke-primary-fixed-dim" />
              
              {/* Dots */}
              <circle cx="20" cy="73" r="1.5" fill="#004782" />
              <circle cx="40" cy="71" r="1.5" fill="#004782" />
              <circle cx="80" cy="39" r="1.5" fill="#004782" />
              <circle cx="100" cy="20" r="1.5" fill="#004782" />
            </svg>
