...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCategories(cats);
        // Take 4 products for featured section
  ...
  const handleHeroSearch = (e) => {
    ...
    if (searchCategory !== "All Categories") params.push(`category=${encodeURIComponent(searchCategory)}`);
  ...
  return (
    <div className="animate-[fade-in_0.3s_ease-out]">
      {/* Hero Section */}
      <section className="relative bg-surface dark:bg-surface-dim overflow-hidden py-xxl transition-colors duration-300">
        <div className="absolute inset-0 medical-pattern"></div>
        <div className="relative max-w-max-width mx-auto px-margin-desktop flex flex-col items-center text-center">
          <span className="bg-secondary-container text-on-secondary-container dark:bg-secondary-fixed dark:text-on-secondary-fixed border border-secondary/20 px-md py-xs rounded-full font-label-sm text-label-sm mb-md inline-block uppercase tracking-wider">
            Reliable Healthcare
          </span>
          <h1 className="font-headline-lg text-headline-lg md:text-5xl text-primary dark:text-primary-fixed-dim max-w-2xl mb-lg">
            Your Health, Delivered Fast
          </h1>
          
          <form onSubmit={handleHeroSearch} className="w-full max-w-3xl flex flex-col md:flex-row items-stretch bg-surface-container-lowest dark:bg-inverse-surface rounded-xl shadow-lg border border-outline-variant dark:border-outline/40 p-sm gap-sm">
            <div className="relative flex-[1.5] flex items-center group">
              ...
              <input
                type="text"
                placeholder="Search medicines, vitamins, or health concerns..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-xl pr-md py-md bg-transparent border-none focus:ring-0 font-body-md text-body-md text-on-surface"
              />
            </div>
            
            <div className="hidden md:block w-px bg-outline-variant dark:bg-outline/40 my-base"></div>
            
            <select
              value={searchCategory}
              onChange={(e) => setSearchCategory(e.target.value)}
              className="flex-1 bg-transparent border-none focus:ring-0 font-body-sm text-body-sm text-on-surface-variant dark:text-surface-variant cursor-pointer py-md px-md dark:bg-inverse-surface"
            >
            ...
            <button type="submit" className="bg-primary text-on-primary px-xl py-md rounded-lg font-label-md text-label-md font-bold hover:bg-primary-container transition-colors shadow-sm active:scale-95">
              Shop Now
            </button>
          </form>
          
          <div className="mt-xl flex flex-wrap justify-center gap-md text-on-surface-variant dark:text-surface-variant">
            <div className="flex items-center gap-xs">
              <span className="material-symbols-outlined text-secondary" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
              <span className="font-label-md">FDA Approved Brands</span>
            </div>
            <div className="flex items-center gap-xs">
              <span className="material-symbols-outlined text-secondary" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
              <span className="font-label-md">Next-day Delivery</span>
            </div>
            <div className="flex items-center gap-xs">
              <span className="material-symbols-outlined text-secondary" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
              <span className="font-label-md">24/7 Pharmacist Support</span>
            </div>
          </div>
        </div>
      </section>
