...
const HealthSupplementsPage = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch Health Supplements", err);
    } finally {
      setLoading(false);
    }
  }, [currentPage]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-margin-mobile md:px-margin-desktop py-xl animate-[fade-in_0.3s_ease-out] text-left">
      ...
      <nav className="flex items-center text-[11px] text-slate-400 gap-xs mb-sm font-semibold select-none">
        ...
        <span className="text-[#004782] dark:text-[#a4c9ff]">Health Supplements</span>
      </nav>

      {/* Header Banner */}
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-r from-[#038076] via-[#0062a3] to-[#004782] text-white p-lg sm:p-xl md:p-xxl mb-lg shadow-lg border border-white/10">
        <div className="relative z-10 max-w-2xl space-y-xs">
          <div className="inline-flex items-center gap-xs px-md py-1 bg-white/10 backdrop-blur-md rounded-full text-xs font-bold text-teal-100 border border-white/20 mb-xs">
            <Sparkles size={14} />
          ...
          <p className="text-xs sm:text-sm text-slate-100 leading-relaxed font-medium">
            Explore premium quality vitamins, nutritional supplements, and daily health &amp; wellness formulations.
          </p>
        </div>
      </div>
